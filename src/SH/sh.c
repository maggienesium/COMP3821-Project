...
void setHorspoolSearch(char* text, uint64_t textLength, Pattern* patterns, int numPatterns,
    int* shiftTable, int minLength, FILE* alertFile) {
    uint64_t pos = 0;

    while (pos <= textLength - minLength) {
        uint64_t windowEnd = pos + minLength - 1;
        int foundMatch = 0;
        ...
        for (int p = 0; p < numPatterns; p++) {
            ...
            for (uint64_t j = patternLen - 1; j >= 0; j--) {
                totalComparisons++;
...
void performSetHorspool(char* text, uint64_t textLength, Pattern* patterns, int numPatterns,
    ...
    ruleCount *topRules = malloc(numPatterns * sizeof(ruleCount));
    if (!topRules) {
        fprintf(stderr, "Memory allocation failed for topRules\n");
        exit(EXIT_FAILURE);
    }
